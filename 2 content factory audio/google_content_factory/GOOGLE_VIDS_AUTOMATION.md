...
Смотри актуальный файл `vids_automation.py` в этой директории. Основные этапы:
...
2. **Сохрани сессию Google**:
   Запусти `save_auth.py`. Откроется браузер, войди в свой Google аккаунт вручную. После входа нажми Enter в терминале. Файл `google_auth.json` будет создан.
   ```bash
   python google_content_factory/save_auth.py
   ```

3. **Запусти автоматизацию**:
   Передай свой промпт как аргумент:
   ```bash
   python google_content_factory/vids_automation.py "Создай видео про пользу ИИ для бизнеса"
   ```
...
- `vids_automation.py` — основной скрипт автоматизации.
- `save_auth.py` — скрипт для первичной авторизации.
- `VIDEO_ANALYSIS_PROMPT.md` — промпт для анализа референсов (например, твоего видео "sen sulu").
...
2. **Автоматизация**:
   ...
   - Запусти скрипт:
     ```bash
     python google_content_factory/vids_automation.py "ВСТАВЬ_ПОЛУЧЕННЫЙ_ПРОМПТ_ЗДЕСЬ"
     ```
