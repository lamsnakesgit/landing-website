...
def build_nano_banana_prompt(slide, story_context=None):
    ...
    return (
        ...
        f"Overall visual style: {style or 'High-end editorial social media design'}\n\n"
        "Use the following on-screen text EXACTLY as written. Preserve language, wording, spelling, and punctuation. "
        "Do not invent extra text, captions, logos, or watermarks. Make the text large, readable, elegant, and typo-free.\n"
        f"ON-SCREEN TEXT: {script_text or 'Новый слайд'}\n\n"
        f"Visual concept: {visual_prompt or 'Clean premium story composition'}\n"
        f"Layout instructions: {layout_instructions or 'Clear hierarchy, readable text, premium composition'}\n"
