  ...
  try {
    ...
    if (update.callback_query) {
      ...
      if (data === 'main_menu') {
        ...
        const { data: user } = await supabase.from('users').select('tokens, username, referred_by').eq('telegram_id', chatId).single();
    ...
    const { data: existingUser } = await supabase.from('users').select('state, state_data, referred_by').eq('telegram_id', chatId).single();
