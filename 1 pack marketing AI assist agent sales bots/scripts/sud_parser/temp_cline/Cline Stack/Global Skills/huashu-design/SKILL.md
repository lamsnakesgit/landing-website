...
### App / Web 原型补充护栏（UI best practices）

如果你做的是 **App 原型、Dashboard、设置页、表单流、可点击 Web 原型**，除了视觉质量，还必须补齐这些行为层约束：

- **状态完整性**：不要只做 happy path。至少显式考虑 `loading`、`empty`、`no results`、`error`、`success`、`disabled`、`permission`、`first-use` 这些状态里哪些和任务相关。
- **空状态替换真实容器**：空状态应该替换掉原本会出现的数据容器，不要保留一个空表格/空卡片骨架再在下面塞提示文案。
- **表单可理解**：表单必须有一直可见的 label；placeholder 不能承担 label 的职责。错误提示要就近出现，并明确告诉用户怎么修。
- **交互可操作**：关键操作不能只靠 hover 暗示；触控目标要够大；放大/缩放后结构不能崩。
- **控件优先级**：能用原生控件就先用原生控件；必须自定义 Dialog / Select / Menu / Tabs / Combobox 时，优先参考 `WAI-ARIA APG`、Radix 一类已验证模式，而不是自己发明键盘交互模型。
