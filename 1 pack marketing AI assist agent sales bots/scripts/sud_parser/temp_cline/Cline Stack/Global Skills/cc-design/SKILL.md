...
**P1.6: Product UI Must Cover Real States.** When the artifact is an app surface, dashboard, settings page, form, data table, or clickable prototype, do not stop at the happy path. Explicitly account for `loading`, `empty`, `no results`, `error`, `success`, `disabled`, `permission`, and `first-use` states where relevant.

- Empty states should replace the absent content structure, not sit underneath empty tables or noisy chrome.
- Each state should guide the user toward one clear next step when one exists.
- Do not hide recovery behind vague copy. If a user can fix something, tell them how.

**P1.7: Operability Beats Surface Polish.** App-like HTML artifacts must preserve visible labels, logical source order, keyboard/focus behavior, zoom resilience, and non-color cues.

- Do not rely on placeholder-only labels.
- Do not rely on hover-only explanations for critical actions.
- Prefer native controls or proven accessible primitives before inventing a custom interaction model.
