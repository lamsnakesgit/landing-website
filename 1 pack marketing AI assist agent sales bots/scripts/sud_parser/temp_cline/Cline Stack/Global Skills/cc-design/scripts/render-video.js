...
(async () => {
  ...
  if (hasReady) {
    // Second defense: actively reset animation time to zero — handles HTML that doesn't
    // strictly follow the starter tick template (e.g., lastTick uses performance.now()
    // causing font loading time to be included in first frame dt)
