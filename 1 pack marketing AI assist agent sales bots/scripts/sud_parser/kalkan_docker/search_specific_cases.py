...
def search_global_cases(query: str):
    ...
    print(f"🔍 Открываем Банк судебных актов. Поиск: {query}")
    ...
    print(f"📡 Отправляем запрос на поиск...")
    ...
    resp_search_post = session.post(f"{BASE_URL}/form/courtActs/index.xhtml", headers=ajax_headers, data=data)
...
if __name__ == "__main__":
    search_global_cases(sys.argv[1] if len(sys.argv) > 1 else "подрядчик")
