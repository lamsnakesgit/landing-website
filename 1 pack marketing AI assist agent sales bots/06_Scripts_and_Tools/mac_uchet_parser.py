    ...
    with sync_playwright() as p:
        ...
        for keyword in niches:
            ...
            try:
                ...
                # Ищем первое поле ввода
                search_input = page.locator('input').first
                try:
                    search_input.wait_for(timeout=15000)
                except:
                    print("⚠️ Поле поиска не найдено. Жду 10 секунд...")
                    time.sleep(10)
                
                # Вводим текст и жмем Enter
                ...
                # Собираем ссылки
                links = []
                all_links = page.locator('a[href*="/c/"]').all()
                for el in all_links:
                    href = el.get_attribute("href")
                    if href and href not in links:
                        links.append(f"https://pk.uchet.kz{href}" if href.startswith('/') else href)
                        
                print(f"✅ Найдено компаний по '{keyword}': {len(links)}. Сканируем первые {limit_per_niche}...")
                ...
                for url in links[:limit_per_niche]:
                    try:
                        ...
                        for b in blocks:
                            try:
                                text = b.inner_text()
                                if 'Первый руководитель' in text or 'Руководитель' in text:
                                    lines = text.split('\n')
                                    for i, line in enumerate(lines):
                                        if 'Первый руководитель' in line or 'Руководитель' in line:
                                            if i + 1 < len(lines):
                                                lpr = lines[i+1].strip()
                                                break
                            except: pass
...
if __name__ == "__main__":
    niches = ["ресторан", "логистика", "строительная компания"]
