...
prompt = 'A photorealistic vertical close-up video of a confident 30-year-old male construction expert wearing a neat black polo shirt, standing in a bright modern apartment. He looks directly at the camera and says in Russian "Затопили соседи? Восстановим за один день без грязи и пыли. Переходите по ссылке, чтобы получить бесплатный замер". High quality, realistic lip movements, cinematic lighting. No text overlays.'
...
print("Reading start frame from Scene 1...")
with open("scene1_last_frame.jpg", "rb") as f:
    img_bytes = f.read()
...
print("Starting generation for Scene 2 with 9:16 and seamless start frame...")
try:
    ...
    if operation.error:
        ...
        for i, video in enumerate(operation.result.generated_videos):
            VIDEO_PATH = "scene2_avatar_9_16.mp4"
            with open(VIDEO_PATH, "wb") as f:
                f.write(video.video.video_bytes)
            print(f"Saved {VIDEO_PATH}!")
            
            LAST_FRAME_PATH = "scene2_last_frame.jpg"
            ...
            body, boundary = multipart_form(
                {"chat_id": CHAT_ID, "caption": "Сцена 2 (бесшовное продолжение от кадра Сцены 1) 🚀\nТекст: 'Затопили соседи? Восстановим за один день без грязи и пыли. Переходите по ссылке, чтобы получить бесплатный замер'"},
                {"video": VIDEO_PATH},
            )
            ...
            if os.path.exists(LAST_FRAME_PATH):
                body2, boundary2 = multipart_form(
                    {"chat_id": CHAT_ID, "caption": "Последний кадр 2-й сцены"},
