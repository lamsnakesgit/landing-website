...
def main():
    for i, prompt in enumerate(PROMPTS, 1):
        print(f"Generating slide {i} with Vertex Imagen 3...")
        img_bytes = generate_with_vertex(prompt)
        if img_bytes:
            filename = f"vertex_final_slide_{i}.png"
            with open(filename, "wb") as f:
                f.write(img_bytes)
            print(f"Sending slide {i} to TG...")
            send_to_tg(img_bytes, "2 22")
            print(f"Done slide {i}")
        time.sleep(3)
