import os, sys, time, json, requests, base64, telegram
...
# Service account для Vertex AI ($190 free trial)
SA_PATHS = [
    'vertex_sa.json',
    '/Users/higherpower/Desktop/1_Active_Projects/2 Ai_agents/2 accounts sub security/amocrm_wazzup_analytics/google_service_account.json',
]
TG_BOT_TOKEN = os.getenv('ANTIGRAVITY_BOT_TOKEN', '6240637727:AAGZsu3ZOAinWoAJwTP11eJJv2M4qzCtx_g')
TG_CHAT_ID = '888005446'
MODEL = 'gemini-3.1-flash-image'  # Nano Banana 2 = Gemini 3.1 Flash Image
LOCATION = 'global'  # Только global endpoint для Nano Banana 2!
...
ENDPOINT = f'https://aiplatform.googleapis.com/v1/projects/{PROJECT_ID}/locations/{LOCATION}/publishers/google/models/{MODEL}:generateContent'
print(f'🔑 Авторизован: project={PROJECT_ID}, model={MODEL}')

PROMPTS = [
    {
        "num": 1,
        "prompt": "Vertical 3:4 aspect ratio. Hyper-viral Instagram feed cover, highly clickable and catchy. A cinematic close-up mugshot of a sleek humanoid robot in an orange prison uniform, looking directly at the viewer with glowing red eyes. The robot holds a large sign with big bold viral typography text \"CLAUDE ЗАБАНИЛИ. КТО ДАЛЬШЕ?\" and smaller readable typography text below it: \"Сначала они молча забанили тысячи аккаунтов Claude. Без объяснения причин. Ты думаешь, это просто сбой? Нет, это предупреждение. Кто следующий?\". Cyberpunk neon prison lighting, gangster movie aesthetic, dramatic shadows. At the very bottom center, small clear typography text \"@lamanopro_ x @aiconicvibe ✔️\".",
        "output": os.path.join(FOLDER, "nano_slide_1.png"),
    },
    {
        "num": 2,
        "prompt": "Vertical 3:4 aspect ratio. A futuristic prison cell door slamming shut. A stylish AI mobster sitting inside behind glowing red laser bars. **CRITICAL: The text on the wall MUST be LARGE, BOLD and VERY LEGIBLE covering at least 30% of the image.** Big bold neon typography graffiti glowing on the wall reads: \"ШАГ ВЛЕВО — ПЕРМАБАН\" and below it in LARGE readable text: \"Цензура ИИ дошла до абсурда. Модели отказываются писать код. Шаг влево — пермабан без возврата денег. Твой бизнес может встать в любую секунду.\" Thick bold font, very large lettering. Gritty, cinematic gangster style, dark moody atmosphere. At the very bottom center, small clear typography text \"@lamanopro_ x @aiconicvibe ✔️\".",
        "output": os.path.join(FOLDER, "nano_slide_2.png"),
    },
    {
        "num": 3,
        ...
        "output": os.path.join(FOLDER, "nano_slide_3.png"),
    },
    {
        "num": 4,
        ...
        "output": os.path.join(FOLDER, "nano_slide_4.png"),
    },
    {
        "num": 5,
        ...
        "output": os.path.join(FOLDER, "nano_slide_5.png"),
    },
    {
        "num": 6,
        "prompt": "Vertical 3:4 aspect ratio. A dark VIP mafia room. A mysterious figure slides a glowing briefcase across a poker table. The briefcase is open, glowing intensely blue from the inside. **CRITICAL: The text MUST be VERY LARGE, BOLD and fill the background — at least 40% of image area.** Big bold neon typography text in the background reads: \"ПИШИ СЛОВО: ОТКРЫТЫЙ\" and below it in LARGE text: \"Хочешь свой независимый, нецензурируемый ИИ? Пиши в Директ кодовое слово ОТКРЫТЫЙ. Я скину секретную подборку топовых моделей и инструкцию по установке.\" Huge thick font covering majority of the image. Cinematic, highly detailed, moody. At the very bottom center, small clear typography text \"@lamanopro_ x @aiconicvibe ✔️\".",
        "output": os.path.join(FOLDER, "nano_slide_6.png"),
    },
]

def generate_image(prompt_text: str, attempt: int = 1, max_retries: int = 5) -> Optional[bytes]:
    """Генерация изображения через Nano Banana 2 (Gemini 3.1 Flash Image) Vertex AI."""
    for retry in range(max_retries):
        ...
        try:
            ...
            if r.status_code == 429:
                wait = 30 * (retry + 1)  # 30, 60, 90, 120, 150 сек
...
def main():
    print(f'🍌 Начинаю генерацию 6 слайдов через {MODEL}')
    ...
    for slide in PROMPTS:
        ...
        if image_bytes:
            with open(slide['output'], 'wb') as f:
                f.write(image_bytes)
            print(f'  ✅ Сохранено: {slide["output"]} ({len(image_bytes)//1024}KB)')
            send_to_tg(image_bytes, f'Слайд {num}/6 — Nano Banana 2')
            results.append(num)
        else:
            print(f'  ❌ Не удалось сгенерировать слайд {num}')
        time.sleep(15)  # пауза между запросами (rate limit friendly)
    
    print(f'\n🎉 Готово! Успешно: {len(results)}/6')
