...
## 6. Обновление от 2026-07-14 (Верификация планировщика и автозапуска)

### Победы (Wins):
1. **Проверка планировщика launchd:** Подтверждена активность LaunchAgent-задачи `com.higherpower.daily_leadgen`. Задача ежедневно в 09:00 AM запускает обертку `~/run_daily_pipeline_wrapper.sh`.
2. **Верификация обхода TCC:** Использование `osascript` для вызова терминала внутри `launchd`-задачи подтвердило свою надежность — скрипты имеют доступ к файловой системе macOS без блокировки со стороны песочницы TCC.
3. **Стабильный резервный ИИ-канал:** Проверена цепочка авторизации и обогащения через Vertex AI (Gemini 2.5 Flash). Благодаря кэшированию токенов OAuth2 и прямым запросам по HTTP, обогащение 10 лидов занимает менее 1 минуты с нулевым риском зависания сети.
4. **Контроль дискового пространства:** Логика автоматической ротации в `run_pipeline.py` успешно удаляет архивные папки лидов и логи старше 7 дней, гарантируя, что дисковая квота хоста не будет исчерпана.

### Issues & Solutions:
* **Проблема:** В `.env` содержатся недействительные ключи для OpenAI/AIHubMix (401 Unauthorized), а также отсутствуют ключи для Supabase.
* **Решение:** Пайплайн автоматически пропускает запись в Supabase с выводом Warning в консоль, а при ИИ-обогащении быстро переключается на Vertex AI с локальным файлом `vertex_sa.json`. Для возобновления работы по OpenAI/Supabase пользователю достаточно прописать актуальные ключи в `.env`.
